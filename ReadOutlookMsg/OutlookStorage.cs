...
namespace iwantedue
{
	public class OutlookStorage : IDisposable {
		...
		internal protected class NativeMethods {
		...
		private class ReferenceManager {
			...
			~ReferenceManager() {
				System.Diagnostics.Trace.WriteLine(String.Format("Destroying Reference Manager: {0} references", trackingObjects.Count));
		...
		public class Recipient : OutlookStorage {
			...
			private Recipient(NativeMethods.IStorage storage)
				: base(storage) {
			}

			internal static Recipient CreateRecipient(NativeMethods.IStorage storage) {
				Recipient r = new Recipient(storage);
				r.propHeaderSize = OutlookStorage.PROPERTIES_STREAM_HEADER_ATTACH_OR_RECIP;
				return r;
			}
			#endregion
		}
		...
		public class Attachment : OutlookStorage {
			...
			/// </summary>
			/// <param name="message">The message.</param>
			private Attachment(NativeMethods.IStorage storage)
				: base(storage) {
				this.propHeaderSize = OutlookStorage.PROPERTIES_STREAM_HEADER_ATTACH_OR_RECIP;
			}

			internal static Attachment CreateAttachment(NativeMethods.IStorage storage) {
				return new Attachment(storage);
			}
			#endregion
		}
		...
		public class Message : OutlookStorage {
			...
			/// <summary>
			/// Gets the date the message was sent or received.
			/// </summary>
			/// <value>The DataTime the message was sent or received.</value>
			public DateTime ReceivedOrSentTime {
				get {
					DateTime d;
					try {
						d = (DateTime)this.GetMapiProperty(OutlookStorage.PR_MESSAGE_DELIVERY_TIME);
					}
					catch {
						d = DateTime.MinValue;
					}
			...
			/// <summary>
			/// Gets the email address of the contact that sent the email.
			/// </summary>
			/// <value>The email address of the contact that sent the email.</value>
			public String FromAddress {
                get { return this.GetMapiPropertyString(OutlookStorage.PR_SENDER_EMAIL); }

                //get
                //{
                //    string fromMail = this.GetMapiPropertyString(OutlookStorage.PR_SENDER_EMAIL);
                //    if (String.IsNullOrEmpty(fromMail) || fromMail.IndexOf('@') < 0)
                //        try
                //        {
                //            //This sometimes throws an exception saying "MAPI property has an unsupported
                //            //type and can not be retrieved."
                //            fromMail = this.GetMapiPropertyString(OutlookStorage.PR_SENDER_EMAIL2);
                //        }
                //        catch
                //        {
                //        }
                //    if (string.IsNullOrEmpty(fromMail) || fromMail.IndexOf('@') < 0)
                //    {
                //        ////Must be an AD account so use the Displayname instead.
                //        //fromMail = From;

                //        ////Get address from active directory.
                //        //fromMail = this.GetMapiPropertyString(OutlookStorage.PR_SENDER_EMAIL);

                //        //System.DirectoryServices.DirectoryEntry root = new System.DirectoryServices.DirectoryEntry("LDAP://" + Environment.UserDomainName);
                //        //string CN = fromMail.Substring(fromMail.LastIndexOf("/CN") + 4);
                //        //fromMail = findEmail(root, CN);
			...
			public String BodyHTML {
				// Note: This property only available for Outlook 2003+. Previous versions did RTF to HTML conversions
				// on the fly.  
				get {
					byte[] htmlBytes = this.GetMapiPropertyBytes(OutlookStorage.PR_HTML);
					if (htmlBytes == null || htmlBytes.Length == 0)
						return null;
					return Encoding.ASCII.GetString(htmlBytes);
				}
			}
			...
			protected override void LoadStorage(NativeMethods.IStorage storage) {
				...
				foreach (ComTypes.STATSTG storageStat in this.subStorageStatistics.Values) {
					...
					if (storageStat.pwcsName.StartsWith(OutlookStorage.RECIP_STORAGE_PREFIX)) {
						Recipient recipient = Recipient.CreateRecipient(subStorage);
			...
			private void LoadAttachmentStorage(NativeMethods.IStorage storage) {
				...
				Attachment attachment = Attachment.CreateAttachment(storage);
        ...
        private const string PR_SENDER_EMAIL = "0C1F"; // "3003";
        private const string PR_SENDER_EMAIL2 = "8013";
		...
		private GCHandle storageHandle;
		...
		private OutlookStorage(string storageFilePath) {
			...
			id = ++counter;
			System.Diagnostics.Trace.WriteLine(string.Format("Creating {2} storage object {0} on path {1}", id, storageFilePath, this.GetType().ToString()));
		...
		private OutlookStorage(Stream storageStream) {
			...
			NativeMethods.ILockBytes memoryStorageBytes = null;
			id = ++counter;
			System.Diagnostics.Trace.WriteLine(string.Format("Creating {1} storage object {0} on memory stream", id, this.GetType().ToString()));
			try {
		...
		private OutlookStorage(NativeMethods.IStorage storage) {
			id = ++counter;
			System.Diagnostics.Trace.WriteLine(string.Format("Creating {1} storage object {0} on loaded storage", id, this.GetType().ToString()));
		...
		/// </summary>
		~OutlookStorage() {
			System.Diagnostics.Trace.WriteLine(string.Format("Deleting storage object {0}", id));
			this.Dispose();
		}
		...
		protected virtual void LoadStorage(NativeMethods.IStorage storage) {
			...
			this.storageHandle = GCHandle.Alloc(this.storage);

			Marshal.AddRef(Marshal.GetIUnknownForObject(storage));
		...
		public virtual void Dispose() {
			System.Diagnostics.Trace.WriteLine(string.Format("Disposing {2} Storage {0}: Already Disposed = {1}", id, disposed, this.GetType().ToString()));
			if (!this.disposed) {
				...
				if (this.storageHandle.IsAllocated)
					this.storageHandle.Free();
